...
interface AutoCategorizeUploadProps {
...
export default function AutoCategorizeUpload({ onDataUpload }: AutoCategorizeUploadProps) {
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/auto-categorize-upload', {
        method: 'POST',
        body: formData,
      });
      ...
      onDataUpload(result.data.url);
      
      // Show success toast
      toast.success(`File "${file.name}" uploaded successfully for auto-categorization!`);
  ...
  const getStatusContent = () => {
    if (uploadStatus.loading) {
      return (
        <div className="flex items-center justify-center">
          ...
          <span className="text-blue-600">Uploading for auto-categorization...</span>
        </div>
      );
    }
    ...
    return (
      <>
        ...
        <p className="text-xs text-gray-500">Files will be uploaded to auto-categorization folder</p>
      </>
    );
  };
  ...
  return (
    ...
    >
      <input
        ...
        className="hidden"
        id="auto-categorize-upload"
        disabled={uploadStatus.loading}
      />
      <label 
        htmlFor="auto-categorize-upload" 
